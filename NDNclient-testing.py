...
async def get_last_frame(device_name):
    ...
    while message_counter < 100:
        try:
            ...
            print(f'Sending Interest {Name.to_str(name)}, {InterestParam(must_be_fresh=True, lifetime=600)}')
            data_name, meta_info, content = await app.express_interest(
                name, must_be_fresh=True, can_be_prefix=True, lifetime=2000)
    ...
    while reconnect_time < 1000:
        ...
        while (lost_frames <= 20) and (current_frame_num < last_frame_num + 10000):
            ...
            if len(frame_queue) < window_len:
                ...
                current_frame_num += 1
            await asyncio.sleep(0.033 - ((time.time() - start_time) % 0.033))
        print("Reconnecting")
        frame_queue.clear()
        start_feed = False
...
def run(device_name, display=False):
    global view_img
    view_img = display
    # Decoding process
    process = (
